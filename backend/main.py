...
web = APIRouter(prefix="/app-music")
...
async def auth_middleware(request: Request, call_next):
    ...
    if path.startswith("/api/") or path.startswith("/app-music/api/"):
        return JSONResponse(status_code=401, content={"error": "Unauthorized"})

    return RedirectResponse(url="/app-music/login")
...
# ═════════════════════════════════════════════════════════════════════════════
# Root-level routes (Alexa, proxy, health, downloader-internal)
# ═════════════════════════════════════════════════════════════════════════════
...
# ── Downloader-internal endpoints (unprotected) ──────────────────────────
...
@app.post("/api/offline/downloader/paused")
async def api_offline_downloader_set_paused_root(request: Request):
    try:
        body = await request.json()
        set_downloader_paused(bool(body.get("paused", True)))
        return {"ok": True}
    except Exception as e:
        logger.error("Set paused error: %s", e)
        return JSONResponse(status_code=400, content={"error": str(e)})
...
if os.path.isdir(static_dir):
    app.mount("/app-music/static", StaticFiles(directory=static_dir), name="static")


# ═════════════════════════════════════════════════════════════════════════════
# Web UI routes (under /app-music)
# ═════════════════════════════════════════════════════════════════════════════
...
<link rel="stylesheet" href="/app-music/static/styles.css?v=4">
<style>
...
async function login() {
  ...
  try {
    const res = await fetch('/app-music/api/login', {
      method: 'POST',
...
@web.get("/privacy", response_class=HTMLResponse)
async def privacy():
...
@web.get("/terms", response_class=HTMLResponse)
async def terms():
...
@web.get("/queue", response_class=HTMLResponse)
async def queue():
...
@web.get("/history/duplicates")
async def history_duplicates():
    return find_duplicates()


@web.post("/history/clean-duplicates")
...
@web.get("/history", response_class=HTMLResponse)
...
<div class="header">
  ...
  <div class="toolbar">
    ...
    <a href="/app-music/history/csv" class="btn btn-outline" style="text-decoration:none;">Exportar CSV</a>
  </div>
</div>
...
async function findDups() {{
  ...
  try {{
    const res = await fetch("/app-music/history/duplicates");
...
async function cleanDups() {{
  ...
  try {{
    const res = await fetch("/app-music/history/clean-duplicates", {{ method: "POST" }});
...
def _build_pagination(page: int, total_pages: int, total: int) -> str:
    ...
    parts.append(f'<a href="/app-music/history?page={page - 1}" class="{prev_class}">Anterior</a>')
    start_p = max(1, page - 2)
    end_p = min(total_pages, page + 2)
    for p in range(start_p, end_p + 1):
        cls = "active" if p == page else ""
        parts.append(f'<a href="/app-music/history?page={p}" class="{cls}">{p}</a>')
    next_class = "disabled" if page >= total_pages else ""
    parts.append(f'<a href="/app-music/history?page={page + 1}" class="{next_class}">Siguiente</a>')
...
@web.get("/api/favorites")
async def api_favorites():
    return get_favorites()


@web.post("/api/favorites")
...
@web.delete("/api/favorites/{video_id}")
async def api_remove_favorite(video_id: str):
    ok = remove_favorite(video_id)
    return {"ok": ok}


@web.get("/api/favorites/check/{video_id}")
...
@web.get("/api/offline/check/{video_id}")
...
async def api_offline_create_task(request: Request):
    try:
        ...
        logger.error("Create offline task error: %s", e)
        return JSONResponse(status_code=400, content={"error": str(e)})


@web.post("/api/offline/downloader/resume")
async def api_offline_downloader_resume():
    set_downloader_paused(False)
    return {"ok": True, "paused": False}


@web.post("/api/offline/clear-completed")
async def api_offline_clear_completed():
    removed = clear_completed_tasks()
    return {"ok": True, "removed": removed}


@web.get("/api/offline/downloader/status")
async def api_offline_downloader_status():
    return {"paused": get_downloader_paused()}
