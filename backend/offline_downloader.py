...
async def process_task(client: httpx.AsyncClient, task: dict):
    ...
    try:
        filepath = await download_song(vid, title, artist)
        await report_status(client, tid, "complete",
...
async def download_song(video_id: str, title: str, artist: str) -> Path:
    ...
    for attempt in range(MAX_RETRIES):
        ...
        logger.warning("yt-dlp attempt %d failed: %s", attempt + 1, err)
